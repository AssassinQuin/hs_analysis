...
class TestManaModifier:
    ...
    def test_coin_gives_temporary_mana(self):
        ...
        coin = _make_card(
            name="The Coin", cost=0, card_type="SPELL",
            text="Gain 1 Mana Crystal this turn.",
            ename="The Coin",
        )
        coin.english_text = "Gain 1 Mana Crystal this turn."
    ...
    def test_preparation_reduces_next_spell(self):
        state = _simple_state(mana=5, max_mana=5)
        prep = _make_card(
            name="Preparation", cost=0, card_type="SPELL",
            text="Your next spell costs (3) less.",
            ename="Preparation",
        )
        prep.english_text = "Your next spell costs (3) less."
...
class TestNewActionTypes:
    ...
    def test_play_with_target_spell(self):
        """测试带目标的法术牌: 通过 v2 CardAbility DamageSpell 造成伤害。"""
        ...
        # 手动注入 v2 CardAbility
        spell.ability = CardAbility(
            on_play=SpellDesc(spell_class="DamageSpell", value=6, target="ENEMY_HERO")
        )
        state.hand.append(spell)
