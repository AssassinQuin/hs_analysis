...
def test_action_hash_deterministic():
    ...
    a = Action(action_type=ActionType.ATTACK, source_index=0, target_index=0)
    assert action_hash(a) == action_hash(a)


def test_action_hash_different_types():
    """Different action types produce different hashes."""
    a1 = Action(action_type=ActionType.ATTACK, source_index=0, target_index=0)
    a2 = Action(action_type=ActionType.PLAY, card_index=0)
...
def test_are_commutative_two_attacks(state):
    ...
    a1 = Action(action_type=ActionType.ATTACK, source_index=0, target_index=0)
    a2 = Action(action_type=ActionType.ATTACK, source_index=1, target_index=0)
    assert are_commutative(a1, a2, state) is True


def test_are_commutative_play_actions(state):
    """PLAY actions are NOT commutative."""
    a1 = Action(action_type=ActionType.PLAY, card_index=0)
    a2 = Action(action_type=ActionType.PLAY, card_index=1)
...
def test_normalize_sorts_commutative_group(state):
    ...
    a1 = Action(action_type=ActionType.ATTACK, source_index=2, target_index=0)
    a2 = Action(action_type=ActionType.ATTACK, source_index=0, target_index=0)
    chromo = [a1, a2]
...
def test_normalize_preserves_non_commutative(state):
    ...
    a1 = Action(action_type=ActionType.PLAY, card_index=1)
    a2 = Action(action_type=ActionType.PLAY, card_index=0)
    chromo = [a1, a2]
    result = normalize_chromosome(chromo, state)
...
def test_is_canonical_true(state):
    ...
    a1 = Action(action_type=ActionType.ATTACK, source_index=0, target_index=0)
    a2 = Action(action_type=ActionType.ATTACK, source_index=2, target_index=0)
    chromo = [a1, a2]
