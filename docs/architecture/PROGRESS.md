...
实现 5 个 FEATURE_GAPS 中记录的缺失功能。**全部 5/5 完成** ✅
...
### Task 3: 对手奥秘概率模型 ✅
- **secret_probability.py**: NEW — 184 行，完整奥秘概率推断引擎
  - `SecretProbabilityModel`: 基于贝叶斯推断 + 场面约束的奥秘概率模型
  - 4 职业 74 张奥秘数据（猎人/法师/圣骑士/潜行者）
  - 触发条件匹配：费用/场面/手牌数/攻击事件 → 排除不可能奥秘
  - 概率排序输出：`get_secret_probabilities()` 返回降序列表
- **global_tracker.py**: 集成 `SecretProbabilityModel` — 对手打出奥秘时自动启动推断
- **packet_replayer.py**: TurnDecision 新增 `opp_secrets_prob` 字段 — 每个决策点输出奥秘概率

### Task 4: 非收藏卡中文名 ✅
- **验证结果**: python-hearthstone XML fallback 已覆盖 98.4% 非收藏卡中文名
- **结论**: 无需代码修改，当前覆盖率满足需求
- **边缘 case**: 仅极少数超新 token 卡缺失，随 python-hearthstone 更新自动补全

### Task 5: 对手卡组类型检测 ✅
- **bayesian_opponent.py**: 新增 `classify_playstyle()` — 从锁定卡组推导打法类型
  - Aggro / Midrange / Control / Combo / OTK 五类
  - 基于卡组曲线+关键词密度+核心卡标识
- **global_tracker.py**: 集成 `classify_playstyle()` — 锁定卡组后自动分类
- **packet_replayer.py**: TurnDecision 新增 `opp_playstyle` 字段 — 对手打法类型输出

### 变更统计

| Task | 文件 | 变更 |
|------|------|------|
| Task 1-2 | 7 files | +451 -39 lines |
| Task 3 | secret_probability.py (NEW, 184 lines) + global_tracker.py + packet_replayer.py | ~250 lines |
| Task 4 | — (无需修改，98.4% zhCN 覆盖已验证) | 0 |
| Task 5 | bayesian_opponent.py + global_tracker.py + packet_replayer.py | ~80 lines |
...
| 18 | Phase 7: 对手智能+效果模拟 (5/5 ✅) | 贝叶斯卡组识别 + 法术目标枚举 + 奥秘概率 + 卡组类型检测 | 676 tests |
...
| *(uncommitted)* | 04-22 | Phase 7 Tasks 3-5: 对手奥秘概率模型 + 非收藏卡中文名验证(98.4%) + 对手卡组类型检测 |
