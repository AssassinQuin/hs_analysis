...
class TestMechanicRegistry:
    def test_register_and_dispatch(self):
        ...
        from hs_analysis.search.engine_v11.mechanics.handler_base import (
            MechanicHandler, ActionContext,
        )

        class TestHandler(MechanicHandler):
        ...
        state = _simple_state()
        ctx = ActionContext(state=state, action=Action(action_type="PLAY"))
        result = registry.dispatch("on_play", state, ctx)
        assert result.hero.armor == 5

    def test_dispatch_unknown_trigger_noop(self):
        from hs_analysis.search.engine_v11.mechanics.registry import MechanicRegistry
        from hs_analysis.search.engine_v11.mechanics.handler_base import ActionContext

        registry = MechanicRegistry()
        state = _simple_state()
        ctx = ActionContext(state=state, action=Action(action_type="PLAY"))
        result = registry.dispatch("nonexistent", state, ctx)
    ...
    def test_handler_exception_graceful(self):
        ...
        class BrokenHandler(MechanicHandler):
            def trigger_point(self):
                return "on_play"
            def apply(self, state, context):
        ...
        registry = MechanicRegistry()
        registry.register("broken", BrokenHandler())
        state = _simple_state()
        ctx = ActionContext(state=state, action=Action(action_type="PLAY"))
        result = registry.dispatch("on_play", state, ctx)
        assert result.hero.hp == 30  # no crash

    def test_default_registry_has_handlers(self):
        from hs_analysis.search.engine_v11.pipeline import _build_default_registry
        registry = _build_default_registry()
        assert registry.handler_count() >= 10
...
class TestFactorGraph:
    def test_empty_evaluator_returns_zero(self):
        from hs_analysis.search.engine_v11.factors.factor_graph import FactorGraphEvaluator
    ...
    def test_all_factors_registered(self):
        from hs_analysis.search.engine_v11.pipeline import _build_default_evaluator
...
class TestAttackPlanner:
    def test_plan_finds_lethal(self):
        from hs_analysis.search.engine_v11.attack_planner import AttackPlanner
...
class TestStrategic:
    def test_lethal_mode(self):
        from hs_analysis.search.engine_v11.strategic import strategic_decision
...
class TestDrawModel:
    def test_empty_deck_negative(self):
        from hs_analysis.search.engine_v11.models.draw_model import DrawModel
        model = DrawModel()
        state = _simple_state()
    ...
    def test_deck_with_cards_positive(self):
        ...
        model = DrawModel()
        state = _simple_state()
        state.deck_list = [_make_card("A", cost=3, v7_score=4.0) for _ in range(5)]
...
class TestDiscoverModel:
    def test_empty_pool(self):
        from hs_analysis.search.engine_v11.models.discover_model import DiscoverModel
        model = DiscoverModel()
        state = _simple_state()
    ...
    def test_pool_picks_best(self):
        ...
        state = _simple_state()
        pool = [
            _make_card("Bad", cost=1, v7_score=1.0),
            _make_card("Good", cost=5, v7_score=6.0),
            _make_card("Medium", cost=3, v7_score=3.5),
            _make_card("Great", cost=7, v7_score=8.0),
        ]
...
class TestRNGModel:
    def test_fixed_damage(self):
        from hs_analysis.search.engine_v11.models.rng_model import RNGModel
        model = RNGModel()
        state = _simple_state()
...
class TestDecisionPipeline:
    def test_simple_development(self):
        from hs_analysis.search.engine_v11.pipeline import DecisionPipeline
...
class TestFactorScores:
    def test_as_dict(self):
        from hs_analysis.search.engine_v11.factors.factor_graph import FactorScores
