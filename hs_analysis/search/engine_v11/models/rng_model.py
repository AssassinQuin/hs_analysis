...
class RNGModel:
    ...
    def _resolve_random(self, effect: str, state: GameState) -> float:
        ...
        heal_match = re.search(r'heal|恢复.*?(\d+)', effect_lower)
        if heal_match:
        ...
        buff_atk = re.search(r'\+(\d+)\s*(?:攻击力|attack)', effect_lower)
        if buff_atk:
            return float(buff_atk.group(1)) * 0.4

        draw_match = re.search(r'draw.*?(\d+)|抽.*?(\d+)', effect_lower)
        if draw_match:
            n = int(draw_match.group(1) or draw_match.group(2))
            return n * 0.5
