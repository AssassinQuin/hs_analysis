...
def test_simple_lethal():
    ...
    state = GameState(
        board=[Minion(name='Giant', attack=15, health=8, max_health=8, can_attack=True)],
...
def test_spell_lethal():
    ...
    state = GameState(
        ...
        hand=[Card(dbf_id=1, name='Fireball', cost=4, card_type='SPELL', text='造成 6 点伤害')],
        opponent=OpponentState(hero=HeroState(hp=6)),
    )
    result = check_lethal(state)
    assert result is not None
    assert any(a.action_type == "PLAY" for a in result)
...
def test_lethal_with_taunt():
    ...
    state = GameState(
        ...
        opponent=OpponentState(
            hero=HeroState(hp=5),
            board=[Minion(name='Taunt', attack=1, health=3, max_health=3, has_taunt=True)],
        ),
    )
    ...
    state2 = GameState(
        board=[
            Minion(name='M1', attack=10, health=10, max_health=10, can_attack=True, has_windfury=True),
        ],
        opponent=OpponentState(
            hero=HeroState(hp=5),
            board=[Minion(name='Taunt', attack=1, health=3, max_health=3, has_taunt=True)],
...
def test_timeout():
    ...
    state = GameState(
        board=[Minion(name=f'M{i}', attack=i+1, health=10, max_health=10, can_attack=True) for i in range(7)],
        opponent=OpponentState(
            hero=HeroState(hp=100),  # way too much health
            board=[Minion(name=f'OT{i}', attack=i+1, health=10, max_health=10, has_taunt=True) for i in range(7)],
        ),
    )
