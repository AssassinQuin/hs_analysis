...
EFFECT_PATTERNS = {
    ...
    "draw":            (r"抽.*?(\d+)\s*张牌",                        lambda m: int(m.group(1)) * 1.2),
    "draw_one":        (r"抽(?:一|1)张牌",                           lambda m: 1.2),
    ...
    "heal_full":       (r"恢复所有?生命值",                           lambda m: 2.0),
    "armor":           (r"获得\s*(\d+)\s*点护甲",                    lambda m: int(m.group(1)) * 0.4),
    "buff_atk":        (r"\+\s*(\d+)\s*.*?攻击力",                   lambda m: int(m.group(1)) * 0.5),
    "buff_health":     (r"\+\s*(\d+)\s*.*?生命值",                   lambda m: int(m.group(1)) * 0.3),
    "buff_both":       (r"\+(\d+)/\+(\d+)",                          lambda m: (int(m.group(1)) + int(m.group(2))) * 0.4),
    "generate":        (r"置入|获取|获得一张|变为|变形",              lambda m: 1.5),
    ...
    "mana_cost_reduce":(r"消耗减少(\d+)",                            lambda m: int(m.group(1)) * 0.6),
    ...
    "corpse":          (r"残骸",                                     lambda m: 0.5),
    "quest":           (r"任务[：:]",                                 lambda m: 1.0),
}
...
CONDITION_DEFS = [
    ("dark_gift",          None, r"黑暗之赐",                                0.6, 1.8),
    ("discover_chain",     "DISCOVER", None,                                0.8, 1.2),
    ("quest_progress",     "QUEST", None,                                   0.7, 3.0),
    ("imbue_stacking",     "IMBUE", None,                                   0.5, 2.0),
    ("reveal_burst",       None, r"回溯",                                    0.6, 1.5),
    ("mana_thirst",        None, r"延系",                                    0.5, 1.6),
    ("trigger_on_turn",    None, r"每当|在你的回合|回合结束|回合开始",          0.7, 1.4),
    ("condition_if",       None, r"如果",                                    0.55, 1.5),
    ("synergy_use_cast",   None, r"使用一张|施放\d+个|打出",                  0.5, 1.3),
    ("discard_payoff",     None, r"弃",                                      0.3, 1.0),
    ("cost_reduction",     None, r"消耗.*减少|每.*减少",                      0.6, 1.4),
    ("copy_effect",        None, r"复制",                                    0.7, 1.3),
    ("draw_enabler",       None, r"抽.*牌",                                  0.8, 1.1),
    ("aoe_clear",          None, r"所有.*(?:伤害|消灭|随从)",                  0.5, 1.5),
    ("buff_aura",          None, r"获得\+\d+/\+\d+|你的.*\+\d+|获得.*攻击力",  0.6, 1.3),
    ("deathrattle_payoff", "DEATHRATTLE", None,                              0.7, 1.3),
    ("battlecry_strong",   "BATTLECRY", None,                               0.9, 1.1),
    ("generate_value",     None, r"置入|获取|获得一张",                       0.7, 1.3),
    ("destroy_removal",    None, r"消灭",                                    0.6, 1.4),
    ("heal_sustain",       None, r"恢复|治疗",                               0.5, 1.1),
    ("armor_gain",         None, r"护甲",                                    0.5, 1.1),
    ("rush_immediate",     "RUSH", None,                                    0.8, 1.2),
    ("charge_lethal",      "CHARGE", None,                                  0.6, 1.3),
    ("taunt_stall",        "TAUNT", None,                                   0.7, 1.1),
    ("freeze_control",     "FREEZE", None,                                  0.5, 1.2),
    ("stealth_setup",      "STEALTH", None,                                 0.5, 1.2),
    ("combo_enabler",      "COMBO", None,                                   0.5, 1.3),
    ("secret_bluff",       "SECRET", None,                                  0.5, 1.2),
    ("tradeable_cycle",    "TRADEABLE", None,                               0.8, 1.1),
    ("condition_race_hand",  None, r"手牌中有.*?(?:" + RACE_NAMES + ")",     0.4, 1.5),
    ("condition_spell_cast", None, r"使用一张.*?法术|施放\d+个法术",          0.6, 1.2),
    ("condition_minion_died",None, r"友方.*?死亡|友方.*?消灭",               0.5, 1.3),
    ("condition_overheal",   "OVERHEAL", None,                              0.4, 1.2),
    ("condition_honorable",  "HONORABLE_KILL", None,                        0.4, 1.3),
    ("forge_chain",          "FORGE", None,                                 0.7, 1.4),
    ("excavate_chain",       "EXCAVATE", None,                              0.6, 1.3),
    ("titan_ability",        "TITAN", None,                                 0.8, 1.2),
]
