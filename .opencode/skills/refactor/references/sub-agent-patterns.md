...
> 重构过程中委派工作给子 Agent 的结构化任务模板。

## 子 Agent 继承规范（强制）

**所有子 Agent 必须在 prompt 中接收以下规范**。向任何子 Agent 下发任务时，将以下块复制到任务描述的开头：

```
【继承规范 — 必须遵守】
1. Framework-First：>500 行文件必须先生成骨架（类定义、方法签名、文档字符串、导入），验证后再填充实现
2. 语言跟随项目：新代码必须匹配项目现有代码风格和约定
3. 测试不可跳过：每个变更必须有验证步骤
4. 原子提交：一个重构一个提交，不混合多个变更
5. 最多 3 个子 Agent：同一时刻最多 3 个子 Agent 并行
6. 无嵌套委派：子 Agent 不得再启动子 Agent，扁平委派
```

## 基本规则

1. **最多 3 个子 Agent** 同时运行 — 避免上下文混乱
2. **任务必须有边界范围** — 明确的输入、输出和完成条件
3. **任务必须独立可验证** — 无需再启动 Agent 即可检查结果
4. **子 Agent 不得嵌套启动子 Agent** — 扁平委派
5. **>500 行文件必须 Framework-First** — 先生成骨架，再填充内容
