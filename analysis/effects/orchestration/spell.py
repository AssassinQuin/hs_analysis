#!/usr/bin/env python3
"""spell.py — Spell Effect Orchestrator.

Layer 3 orchestrator: uses EffectParser for parsing and primitives
...
# Bridge to new parser — returns List[Tuple[str, object]] legacy format
...
from analysis.effects.primitives.modify import apply_silence_to_minion
...
# ===================================================================
# EffectApplier — orchestrator-level atomic operations
# ===================================================================

class EffectApplier:
    """Orchestrator-level effect application primitives.

    Translates effect dispatch into concrete GameState mutations.
    """
    ...
    def apply_silence(state: GameState, target: str) -> GameState:
        ...
        if target.startswith('enemy_minion:'):
            idx = int(target.split(':')[1])
            if 0 <= idx < len(s.opponent.board):
                apply_silence_to_minion(s.opponent.board[idx])
        elif target.startswith('friendly_minion:'):
            idx = int(target.split(':')[1])
            if 0 <= idx < len(s.board):
                apply_silence_to_minion(s.board[idx])
        return s
...
def resolve_effects(state: GameState, card: Card, target_index: int = -1) -> GameState:
    ...
    Orchestrates:
      ...
      3. Apply each effect using EffectApplier
      4. Handle lifesteal
      5. Resolve deaths
        ...
        opponent=OpponentState(
            ...
            board=[
                Minion(dbf_id=3001, name="Voidwalker", attack=1, health=3,
                       max_health=3, owner="enemy", keywords={'TAUNT'}),
