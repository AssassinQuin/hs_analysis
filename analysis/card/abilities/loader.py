...
Usage:
    ...
    python -m analysis.card.abilities.loader --card CS2_029  # inspect card
"""
...
from analysis.card.abilities.spells import Spell, SPELL_REGISTRY
...
def load_card_spells(card_id: str) -> List[Spell]:
    """加载卡牌的 Spell 实例列表（MetaStone 新格式）。

    从 JSON 中读取 abilities[].actions 列表，
    通过 Spell.from_dict() 反射构造 Spell 实例。
    """
    data = _load_json_data()
    cards = data.get("cards", {})
    card_data = cards.get(card_id)
    ...
    for ability in abilities:
        for action_data in ability.get("actions", []):
            spells.append(Spell.from_dict(action_data))
    return spells


def load_card_power(card_id: str) -> dict:
    """加载卡牌的完整能力定义 dict（含 trigger + actions）。

    用于后续 P4 CardPower 构建。
    返回格式: {"trigger": "BATTLECRY", "actions": [...], ...}
    """
    data = _load_json_data()
    cards = data.get("cards", {})
    card_data = cards.get(card_id)
...
def _load_json_data() -> dict:
    ...
    支持两种格式:
    - 新格式: {"version": 1, "cards": {...}} — 直接使用
    - 旧格式: {"cards": {...}} — 无 version 字段，直接使用
    统一返回 data["cards"] 存在于 data 中的结构。
    """
    global _cache
    ...
    try:
        ...
        data = json.loads(raw)
        # 新格式有 version 字段，兼容处理
        if "version" in data and "cards" in data:
            _cache = data
        elif "cards" in data:
            # 旧格式或简单格式，直接使用
            _cache = data
            ...
            # 无 cards 字段，当作空数据处理
            log.warning("JSON 数据中缺少 'cards' 字段，返回空 dict")
            _cache = {}
...
def _parse_effect(data: dict) -> EffectSpec:
    ...
    from analysis.card.abilities.value_expr import to_lazy_value
...
def _serialize_abilities(abilities: List[CardAbility]) -> list:
    ...
    from analysis.card.abilities.value_expr import from_lazy_value
...
if __name__ == "__main__":
    ...
    if "--build" in args:
        ...
        if idx + 1 < len(args):
            ...
            print("Usage: python -m analysis.card.abilities.loader --card <CARD_ID>")
    else:
        print("Usage:")
        print("  python -m analysis.card.abilities.loader --build          Build JSON from parser")
        print("  python -m analysis.card.abilities.loader --card <CARD_ID> Inspect a card")
