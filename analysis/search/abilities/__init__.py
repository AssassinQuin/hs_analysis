...
def enumerate_legal_actions(*args, **kwargs):
    ...
    from analysis.card.engine.rules import enumerate_legal_actions as _fn
    return _fn(*args, **kwargs)


def apply_action(*args, **kwargs):
    """Lazy re-export."""
    from analysis.card.engine.simulation import apply_action as _fn
    return _fn(*args, **kwargs)


def apply_draw(*args, **kwargs):
    """Lazy re-export."""
    from analysis.card.engine.simulation import apply_draw as _fn
    return _fn(*args, **kwargs)


def next_turn_lethal_check(*args, **kwargs):
    """Lazy re-export."""
    from analysis.card.engine.simulation import next_turn_lethal_check as _fn
    return _fn(*args, **kwargs)


def load_abilities(*args, **kwargs):
    """Lazy re-export for JSON ability loader."""
    from analysis.card.abilities.loader import load_abilities as _fn
    return _fn(*args, **kwargs)
