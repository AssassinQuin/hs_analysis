...
# Fallback damage parser when card_effects module is unavailable
_DAMAGE_RE = re.compile(r'(\d+)')


def _fallback_get_card_damage(card) -> int:
    """Parse damage from card text when structured effects are unavailable."""
    text = getattr(card, 'text', '') or ''
    en = getattr(card, 'english_text', '') or ''
    for t in (text, en):
        for m in _DAMAGE_RE.finditer(t):
            return int(m.group(1))
    return 0


if get_card_damage is None:
    get_card_damage = _fallback_get_card_damage
