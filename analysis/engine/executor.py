...
from analysis.engine.deterministic import DeterministicRNG, det_top_k

if TYPE_CHECKING:
    from analysis.engine.state import GameState, HeroState, Minion, Weapon
    from analysis.abilities.definition import CardAbility, AbilityTrigger
...
def _apply_keyword(minion: Any, keyword: str) -> None:
    ...
    Args:
        minion: Minion instance.
        keyword: Keyword name (case-insensitive), e.g. 'TAUNT', 'divine_shield'.
    """
    if not keyword:
        return
    kw = keyword.upper()
    field = _KEYWORD_FIELD_MAP.get(kw)
